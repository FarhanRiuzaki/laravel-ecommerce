...
class CategoryController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($request->ajax())
        {
            $data = Category::with(['parent'])->orderBy('created_at', 'DESC')->get();
            return DataTables::of($data)
                    ...
                    ->addColumn('action', function($data){
                        $button = '<a href="'. route('category.edit', $data->id).'" class="btn btn-warning btn-sm">Edit</a>';
                        $button .= '&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-danger btn-sm btn-delete" data-remote="'. route('category.destroy', $data->id) .'" data-id="'.$data->id.'">Hapus</button>';
                        return $button;
                    })
    ...
    public function destroy($id)
    {
        ...
        // return redirect(route('category.index'))->with(['warning' => 'Kategori Ini Memiliki Anak Kategori!']);
        return response()->json(['code' => 'error', 'msg' => 'Kategori ini memiliki anak kategori!']);
    }
}
